...
func assertObjectDepsEqual(t *testing.T, res object.Object, expectedDeps []string) {
	...
	deps := debugTraceObj.DebugMetadata
...
func TestDependencyTrackingIndexIntoArray(t *testing.T) {
	program := "let f = fn(a, x) { a[x] }; deps(f, [1,2,3,4,5], 2)"
	res := testEval(program)
	assertObjectDepsEqual(t, res, []string{"0|2", "1"})
}

func TestDependencyTrackingInFunctionWithArraysWhichReturnAnArray(t *testing.T) {
	program := "let f = fn(a) { [a[0],a[1],a[2],a[3]] }; deps(f, [5,6,7,8,9])"
	res := testEval(program)
...
func TestDependencyTrackingBasicHash(t *testing.T) {
	program := "let f = fn(a) { a[\"steve\"] }; deps(f, {\"steve\": 3})"
	res := testEval(program)
	assertObjectDepsEqual(t, res, []string{"0|@steve"})
}

func TestDependencyTrackingBasicHashIndex(t *testing.T) {
	program := "let f = fn(a, b) { a[b] }; deps(f, {\"steve\": 3}, \"steve\")"
	res := testEval(program)
	assertObjectDepsEqual(t, res, []string{"0|@steve", "1"})
}

func TestDependencyTrackingHashLength(t *testing.T) {
	program := "let f = fn(a) { len(a) }; deps(f, {\"steve\": 3})"
	res := testEval(program)
	assertObjectDepsEqual(t, res, []string{"0#"})
}

func TestDependencyTrackingHashConcat(t *testing.T) {
	program := "let f = fn(a, b) { len(a + b) }; deps(f, {\"steve\": 3}, {\"grimes\": 5})"
	res := testEval(program)
	assertObjectDepsEqual(t, res, []string{"0#", "1#"})
}
