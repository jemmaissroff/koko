...
func BenchmarkMergeSort(b *testing.B) {
	...
	let repeat_merge_sort_with_modifications = fn(repeats, arr) { if (repeats != 0) { let mod_ind = rando(len(ra)) repeat_merge_sort_with_modifications(repeats - 1, merge_sort(get_n_elements(ra, 0, mod_ind) + [rando(RAND_CONST)] + get_n_elements(ra, mod_ind, len(ra) - mod_ind))) }
	`)
	for i := 0; i < b.N; i++ {
		evaluator.Eval(program, env)
	}
