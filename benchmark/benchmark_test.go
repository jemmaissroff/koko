...
func BenchmarkRockHopper(b *testing.B) {
	...
	let repeat_rock_hopper_with_modifications = fn(repeats, arr) {
		 if (repeats != 0) {
			let mod_ind = rando(len(arr))
			merge_sort(arr)
			repeat_merge_sort_with_modifications(repeats - 1, get_n_elements(ra, 0, mod_ind) + [0] + get_n_elements(ra, mod_ind, len(ra) - mod_ind))
		}
	}
	repeat_merge_sort_with_modifications(200, ra)
	`)
